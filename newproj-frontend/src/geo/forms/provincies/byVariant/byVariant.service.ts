...
@Injectable()
export class ProvinciesByVariantFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    private provinciesService: ProvinciesService,
  ) {
    this.form = new FormGroup({
      variant: new FormControl({value: undefined, disabled: false}, [Validators.required]),
      maxRegs: new FormControl({value: undefined, disabled: false}, []),
    });
  ...
  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.ConjuntElementsProvincia> {
    if (value === false) {
    ...
    this.cacheSub[JSON.stringify(value)] = new ReplaySubject<__model.ConjuntElementsProvincia>(1);
  ...
  try(subject: ReplaySubject<__model.ConjuntElementsProvincia>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value)];
      return;
    }
    const result = this.provinciesService.byVariant(value);
    result.pipe(
      map(val => {
