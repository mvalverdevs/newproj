...
@Injectable()
export class ProvinciesByNomFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      nom: new FormControl({value: undefined, disabled: false}, [Validators.required]),
  ...
  try(subject: ReplaySubject<__model.ConjuntElementsProvincia>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    ...
    const result = this.provinciesService.byNom(value);
    result.pipe(
      map(val => {
