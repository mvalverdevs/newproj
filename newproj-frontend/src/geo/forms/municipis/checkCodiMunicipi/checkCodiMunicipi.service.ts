...
@Injectable()
export class MunicipisCheckCodiMunicipiFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      ...
      comarca: new FormControl({value: undefined, disabled: false}, [Validators.required]),
  ...
  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<boolean> {
    if (value === false) {
    ...
    this.cacheSub[JSON.stringify(value)] = new ReplaySubject<boolean>(1);
  ...
  try(subject: ReplaySubject<boolean>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value)];
      return;
    }
    const result = this.municipisService.checkCodiMunicipi(value);
    result.pipe(
      map(val => {
