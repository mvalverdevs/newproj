...
@Injectable()
export class MunicipisVariantsMunicipisProvFormService {
  form: FormGroup;
  defaultValue: any;
  ...
  constructor(
    ...
    this.form = new FormGroup({
      provincia: new FormControl({value: undefined, disabled: false}, [Validators.required]),
  ...
  submit(value: any = false, cache: boolean = true, only_cache: boolean = false): Observable<__model.ConjuntElementsVarMunicipi> {
    if (value === false) {
    ...
    this.cacheSub[JSON.stringify(value)] = new ReplaySubject<__model.ConjuntElementsVarMunicipi>(1);
  ...
  try(subject: ReplaySubject<__model.ConjuntElementsVarMunicipi>, value: any, cache_hit: boolean, cache: boolean, waitOnRetry = 1000, maxRetries = environment.apiRetries): void {
    if (JSON.stringify(value) !== JSON.stringify(this.currentValue)) {
      subject.complete();
      delete this.cacheSub[JSON.stringify(value)];
      return;
    }
    const result = this.municipisService.variantsMunicipisProv(value);
    result.pipe(
      map(val => {
