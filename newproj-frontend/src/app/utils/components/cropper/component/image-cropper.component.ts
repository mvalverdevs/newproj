...
export class ImageCropperComponent implements OnChanges {
    ...
    @Input() cropperPositionFullWidth: CropperPosition;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (changes.cropperPosition) {
          this.calculateCropperPosition();
        }
    ...
    private calculateCropperPosition() {
      const sourceImageElement = this.sourceImage;
      if (this.cropperPositionFullWidth && sourceImageElement.nativeElement.offsetWidth > 0){
        const canvasWidth = sourceImageElement.nativeElement.offsetWidth;
        const canvasHeight = sourceImageElement.nativeElement.offsetHeight;

        this.cropper.x1 = this.cropperPositionFullWidth.x1 * canvasWidth / this.originalSize.width;
        this.cropper.x2 = this.cropperPositionFullWidth.x2 * canvasWidth / this.originalSize.width;
        this.cropper.y1 = this.cropperPositionFullWidth.y1 * canvasHeight / this.originalSize.height;
        this.cropper.y2 = this.cropperPositionFullWidth.y2 * canvasHeight / this.originalSize.height;
