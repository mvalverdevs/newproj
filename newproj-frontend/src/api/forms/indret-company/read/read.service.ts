...
@Injectable()
export class IndretCompanyReadFormService extends YASAGGetFormService<__model.IndretCompany> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: IndretCompanyService,
  ) {
    super('IndretCompanyRead', apiConfigService, ngZone);
  ...
  submit(value: any = false, cache = true, only_cache = false): Observable<__model.IndretCompany> {
    const result = val => this.service.read(val);
    return this._submit('__model.IndretCompany', result, value, cache, only_cache );
  }
  listen(value: any = false, submit: boolean = true): Observable<__model.IndretCompany> {
    if (submit) {
      this.submit(value);
    }
    return this._listen('__model.IndretCompany', value, submit);
  }
