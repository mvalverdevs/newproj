...
@Injectable()
export class IndretSamplePointReadFormService extends YASAGGetFormService<__model.SamplePoint> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: IndretSamplePointService,
  ) {
    super('IndretSamplePointRead', apiConfigService, ngZone);
  ...
  submit(value: any = false, cache = true, only_cache = false): Observable<__model.SamplePoint> {
    const result = val => this.service.read(val);
    return this._submit('__model.SamplePoint', result, value, cache, only_cache );
  }
  listen(value: any = false, submit: boolean = true): Observable<__model.SamplePoint> {
    if (submit) {
      this.submit(value);
    }
    return this._listen('__model.SamplePoint', value, submit);
  }
