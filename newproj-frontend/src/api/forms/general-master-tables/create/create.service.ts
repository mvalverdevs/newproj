...
@Injectable()
export class GeneralMasterTablesCreateFormService extends YASAGPostFormService<__model.GeneralMasterTables> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: GeneralMasterTablesService,
  ) {
    super('GeneralMasterTablesCreate', apiConfigService, ngZone);
    this.form = new FormGroup({
      data: new FormGroup({
        id: new FormControl({value: undefined, disabled: false}, []),
        is_multiple: new FormControl({value: false, disabled: false}, []),
