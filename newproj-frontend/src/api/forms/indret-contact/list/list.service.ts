...
@Injectable()
export class IndretContactListFormService extends YASAGGetFormService<__model.IndretContactList> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: IndretContactService,
  ) {
    super('IndretContactList', apiConfigService, ngZone);
  ...
  submit(value: any = false, cache = true, only_cache = false): Observable<__model.IndretContactList> {
    const result = val => this.service.list(val);
    return this._submit('__model.IndretContactList', result, value, cache, only_cache );
  }
  listen(value: any = false, submit: boolean = true): Observable<__model.IndretContactList> {
    if (submit) {
      this.submit(value);
    }
    return this._listen('__model.IndretContactList', value, submit);
  }
