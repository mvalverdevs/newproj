...
@Injectable()
export class DocumentLibraryPartialUpdateFormService extends YASAGPostFormService<__model.DocumentLibrary> {
  constructor(
    apiConfigService: APIConfigService,
    ngZone: NgZone,
    private service: DocumentLibraryService,
  ) {
    super('DocumentLibraryPartialUpdate', apiConfigService, ngZone);
  ...
  submit(value: any = false): Observable<__model.DocumentLibrary> {
    ...
    return this._submit('__model.DocumentLibrary', result, 'data', value, true );
  }
